...
def extract_date_features(data):
    """Extract date features from 'dt_opened'"""
    if 'dt_opened' in data.columns:
        data['dt_opened'] = pd.to_datetime(data['dt_opened'], format='%d-%m-%Y', errors='coerce')
        data['year_opened'] = data['dt_opened'].dt.year
        data['month_opened'] = data['dt_opened'].dt.month
        data['day_opened'] = data['dt_opened'].dt.day
        data['hour_opened'] = data['dt_opened'].dt.hour
        data = data.drop(columns=['dt_opened'])
        logging.info("Extracted 'dt_opened' date features.")
    return data
